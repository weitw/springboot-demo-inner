...
public class DynamicScheduledTaskService {
    ...
    @PostConstruct
    public void init() {
        currentCronExpression = jobInfoMapper.findCronByUuid("dynamic1"); // 从数据库获取cron表达式
        scheduleTask(currentCronExpression);
    }

    public void scheduleTask(String currentCron) {
        if (futureTask != null) {
            futureTask.cancel(true); // 取消之前的任务
        }
        futureTask = taskScheduler.schedule(this::executeTask1, new CronTrigger(currentCron)); // 使用新的cron表达式调度任务
    }

    public void executeTask1() {
        // 这里是你的任务执行逻辑
        log.info("任务1执行, cron={}, now={}", currentCronExpression, DateUtils.normalDateFormat(new Date()));
    }

    // 可以定期调用此方法来检查数据库中的cron表达式是否有变化
    @Scheduled(fixedDelay = 30000) // 例如，每分钟检查一次
    public void checkAndUpdateCronExpression() {
        String newCronExpression = jobInfoMapper.findCronByUuid("dynamic1");
